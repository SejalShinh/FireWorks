...
public class Firework {
	...
	private Firework() {
		...
		Firework.numberOfParticlesCreated++;
	...
	public static Firework pearls(int n, Point2 position, double speed) {
		...
		for (int i = 0; i < n; i++) 
		{
		double valx = Firework.rng.nextDouble();
		double valy = Firework.rng.nextDouble();
        
		valx = (position.getX() + (valx - 0.5) * 30);
        valy = (position.getY() + (valy - 0.5) * 60);
        
        Point2 newpos = new Point2(valx, valy);
        Vector2 vel = new Vector2(0.0, 1.0);
	...
	public static Firework rings(Point2 position, double speed) {
		...
		for (int i = 0; i < 360; i += 10) {
			...
			Particle out = new Particle(position, vel, 3, Color.BLUE);
			f.particles.add(out);
		}
		...
		for (int i = 0; i < 360; i += 10) {
			...
			Particle in = new Particle(position, vel, 2, Color.RED);
			f.particles.add(in);
		}
	...
	public static Firework fountain(int n, Point2 position, double speed) {
		...
		for (int i = 0; i < n; i++) 
		{
			double norm = Firework.rng.nextGaussian() * 2 + 90;
			Vector2 vel = Vector2.dirVector(norm);
			double s = Firework.rng.nextGaussian() * (0.05 * speed) + speed;
			vel.multiply(s);
		...
		Firework.numberOfFireworksCreated += f.particles.size();
		return f;
	...
	public static Firework pistil(Point2 position, double speed) {
		...
		// to implement
			...
			for (int i = 0; i < 500; i++) 
			{
				double startvel = Firework.rng.nextDouble() * 360 + 0;
				Vector2 vel = Vector2.dirVector(startvel);
				double s = Firework.rng.nextDouble() * (0.3 * speed) + 0;
				vel.multiply(s);
				Particle in = new Particle(position, vel, 2, Color.GREEN);
				f.particles.add(in);
			}
			Firework.numberOfFireworksCreated += f.particles.size();
			return f;
